...
from sphereclusterIntensity import spherical_kmeans
...
def _maximization(X, posterior, force_weights=None, Int=None):
    ...
    for cc in range(n_clusters):
        ...
        if force_weights is None:
            if Int is None:
                #print('in force_weights: no Intensity supplied')
                weights[cc] = np.mean(posterior[cc, :])
            else:
                #print('in force_weights: with Intensity supplied')
                weights[cc] = sum(posterior[cc, :] * Int) / sum(Int)
        else:
        ...
        if sp.issparse(X):
            ...
            if Int is None:
                #print('in sp.issparse(X): no Intensity supplied')
                for ee in range(n_examples):
                    X_scaled[ee, :] *= posterior[cc, ee]
            else:
                #print('in sp.issparse(X): with Intensity supplied')
                for ee in range(n_examples):
                    X_scaled[ee, :] *= posterior[cc, ee] * Int[ee]
        ...
        if Int is None:
            #print('in rbar: no Intensity supplied')
            rbar = center_norm / (n_examples * weights[cc])
        else:
            #print('in rbar: with Intensity supplied')
            rbar = center_norm / (sum(Int) * weights[cc])
...
def _movMF(X, n_clusters, posterior_type='soft', force_weights=None,
           ...
           random_state=None, tol=1e-6, Int=None):
...
def movMF(X, n_clusters, posterior_type='soft', force_weights=None, n_init=10,
          ...
          random_state=None, tol=1e-6, copy_x=True,Int=None):
...
class VonMisesFisherMixtureInt(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def fit(self, X, Int=None, y=None):
